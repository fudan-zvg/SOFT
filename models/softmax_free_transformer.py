...
class SoftmaxFreeAttentionKernel(nn.Module):
    def __init__(self, dim, num_heads, q_len, k_len, num_landmark, use_conv, max_iter=30):
        super().__init__()
    ...
    def forward(self, Q, V):
        ...
        if self.num_landmarks == self.q_seq_len:
            ...
            attn = subtraction_gaussian_kernel(Q_landmarks, K_landmarks.transpose(-1, -2).contiguous())
            ...
            kernel_1_ = subtraction_gaussian_kernel(Q, K_landmarks.transpose(-1, -2).contiguous())
            kernel_1_ = torch.exp(-kernel_1_/2)

            kernel_2_ = subtraction_gaussian_kernel(Q_landmarks, K_landmarks.transpose(-1, -2).contiguous())
    ...
    def newton_inv(self, mat):
        ...
        if self.init_option == "alpha":
            ...
            while pnorm > 1.01 and err_cnt < 10:
                ...
                pnorm = torch.max(torch.sum(torch.abs(I - torch.matmul(P, V)), dim=-2))
                err_cnt += 1
        elif self.init_option == "original":
            # This original implementation is more conservative to compute coefficient of Z_0.
            V = 1 / (torch.max(torch.sum(P, dim=-2)) * torch.max(torch.sum(P, dim=-1))) * P.transpose(-1, -2)
        else:
            # This is the exact coefficient computation, 1 / ||K||_1, of initialization of Z_0, leading to faster convergence.
            V = 1 / (torch.max(torch.sum(P, dim=-2), dim=-1).values *
                     torch.max(torch.sum(P, dim=-1), dim=-1).values)[:, :, None, None] * P.transpose(-1, -2)
    ...
    def iterative_inv(self, mat, n_iter=6):
        ...
        # kernel_2_softmax_cp = mat[0][0].detach().cpu().numpy().astype(np.float32)
        # inverse_np = linalg.inv(kernel_2_softmax_cp)
        # z0 = V[0][0].detach().cpu().numpy().astype(np.float32)
        # diff = np.matmul(kernel_2_softmax_cp, inverse_np) - np.matmul(kernel_2_softmax_cp, z0)
        # print("norm: ", linalg.norm(diff, ord=2))
        # input()
...
class SoftmaxFreeAttention(nn.Module):
    def __init__(self, dim, num_heads, q_len, k_len, num_landmark, conv_size, max_iter=30):
        super().__init__()
...
class SoftmaxFreeTransformer(nn.Module):
    def __init__(self, dim, num_heads, q_len, k_len, num_landmark, conv_size, drop_path=0., max_iter=30):
        super().__init__()
        self.dim = dim
        self.hidden_dim = int(4*dim)

        self.mha = SoftmaxFreeAttention(dim, num_heads, q_len, k_len, num_landmark, conv_size, max_iter)
...
class SoftmaxFreeTrasnformerBlock(nn.Module):
    def __init__(self, dim, num_heads, H, W, drop_path=0., conv_size=3, max_iter=30):
        super().__init__()
        seq_len = 49
        self.att = SoftmaxFreeTransformer(dim, num_heads, int(H*W), int(H*W), seq_len, conv_size, drop_path, max_iter)
